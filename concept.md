...
The React app provides a clear visualization of the blocks (with data like timestamp, hashes, transactions) and may also show wallet balances, etc. It will likely use polling or websockets (if integrated) to update when new data arrives, but in this project we rely on the backend to push updates via PubNub and the frontend to fetch updated state as needed.
	...
	•	A “TRANSACTION” channel for broadcasting new transactions.
Whenever a node mines a new block, it publishes the block data to the BLOCK channel, so all other nodes can receive it and update their local blockchain (if the received block is valid and extends the chain). Similarly, when a node creates or receives a new transaction, it broadcasts it on the TRANSACTION channel so that other peers can add it to their transaction pool.
...
2. Mining a Block (User/Automated -> Backend -> Network):
	...
	•	The new block’s previous_hash will point to the hash of the latest block in the blockchain.
